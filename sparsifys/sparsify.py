...
def pto_ang_map(velo_points, H=64, W=512, slice=1, line_spec=None, random_sample=0, get_lines=False, fill_in_line=None, fill_in_spec=None, fill_in_slice=None):
    """
    :param fill_in_slice:
    :param fill_in_spec:
    :param fill_in_line:
...
def gen_sparse_points(data_idx, args):
    ...
    if args.store_line_map_dir is not None:
        depth_map_lines, ptc = pto_ang_map(pc_velo, H=args.H, W=args.W, slice=args.slice, line_spec=args.line_spec, random_sample=args.random_sample, get_lines=True,
...
def gen_sparse_points_all(args):
    with open(args.split_file) as f:
        data_idx_list = [x for x in f.readlines() if len(x.strip()) > 0]
    ...
    for data_idx in data_idx_list:
        res.append((data_idx, pool.apply_async(sparse_and_save, args=(args, data_idx), callback=update)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ptc_path', type=str,
                        ...
                        default='G:\\Dataset\\KITTI\\rawdata\\')
    ...
    parser.add_argument('--line_spec', type=int, nargs='+', default=[2, 7, 12, 16])
    ...
    parser.add_argument('--split_file', type=str, default="../splits/eigen_zhou/train_files.txt")
